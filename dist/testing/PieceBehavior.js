...
const ArrayUtils_js_1 = require("../public/js/ArrayUtils.js");
...
class PieceParamsTemplate {
    ...
    attackRayCastParams() {
        if (!this._attackRayCastParams) {
            ...
            this._attackRayCastParams = ArrayUtils_js_1.unique(params);
        }
    ...
    rayCastParams() {
        if (!this._rayCastParams) {
            ...
            let params = behaviors.map(b => b.toRayCastParams()).flat();
            this._rayCastParams = ArrayUtils_js_1.unique(params);
        }
...
class PieceBehavior {
    ...
    _expandAxisRules() {
        ...
        const appendToAll = (arrs, item) => arrs.map(inner => inner.concat([item]));
        let result = [[]];
    ...
    _permuteReplace() {
        ...
        const insertToAll = (item, arrs) => arrs.map(inner => [item].concat(inner));
        ...
        let directions = axisRules.map(axisRule => {
            ...
            let uniqueDirs = ArrayUtils_js_1.unique(dirs);
            return uniqueDirs;
        });
        directions = directions.flat();
        return ArrayUtils_js_1.unique(directions);
    }
}
...
PieceBehavior.computeAll();
exports.default = PieceBehavior;
//# sourceMappingURL=PieceBehavior.js.map