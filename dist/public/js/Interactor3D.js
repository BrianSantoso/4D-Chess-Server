...
class Interactor3D extends Receiver_js_1.AbstractReceiver {
    constructor(team, chessGame, player, rayCaster) {
        super(team, chessGame, player);
        this._rayCaster = rayCaster;
        this._movePreviewer = new MovePreviewer(this, ChessTeam_js_1.default.OMNISCIENT);
        this._pieceSelector = new PieceSelector(this, team);
        this._moveConfirmer = new MoveConfirmer(this, ChessTeam_js_1.default.GHOST);
        this._moveExplainer = new MoveExplainer(this, ChessTeam_js_1.default.oppositeTeam(team));
        let trySelectPiece = () => {
...
class Interactor3DWorker {
    ...
    _showPossibleMoves(mesh, preview = false) {
        if (Interactor3D.isPiece(mesh)) {
            ...
            this._boardGraphics().showPossibleMoves(pieceId, moves, preview, config_json_1.default.animFrames.showMoves);
        }
    }
    _hidePossibleMoves(mesh) {
        if (Interactor3D.isPiece(mesh)) {
            let pieceId = mesh.pieceId;
            this._boardGraphics().hidePossibleMoves(pieceId, config_json_1.default.animFrames.hideMoves);
        }
    }
    _highlight(mesh) {
        if (Interactor3D.isPiece(mesh)) {
            let pieceId = mesh.pieceId;
            this._boardGraphics().highlight(pieceId, config_json_1.default.animFrames.highlight);
        }
    }
    _unhighlight(mesh) {
        if (Interactor3D.isPiece(mesh)) {
            let pieceId = mesh.pieceId;
            this._boardGraphics().unhighlight(pieceId, config_json_1.default.animFrames.unhighlight);
        }
    }
...
exports.default = Interactor3D;
//# sourceMappingURL=Interactor3D.js.map